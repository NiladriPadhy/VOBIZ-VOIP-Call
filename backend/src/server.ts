...
app.post(
  ...
  async (request, response) => {
    ...
    if (!isSipLeg) {
      ...
      sendXml(response, dialUserXml(env.SIP_ENDPOINT, caller, did));
      return;
    }
    ...
    sendXml(response, dialNumberXml(destination, callerId));
  },
);
...
function dialNumberXml(destination: string, callerId: string): string {
  return xmlResponse(
...
function dialUserXml(sipEndpoint: string, caller: string, did: string): string {
